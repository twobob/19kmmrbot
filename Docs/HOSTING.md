# Extended Hosting & Deployment Guide

This document covers the full architecture, configuration reference, deployment workflows, and multi-tenant streamer setup for the Fortify Dota Underlords GSI Twitch Bot.

For the quick-start guide, see the main [README](../README.md).
...
## Windows Setup (From Scratch - Automated)
...
*   **Graceful Failure Handling:** The bot safely catches Twitch connection failures if credentials are not configured yet, keeping the Express GSI server alive on port `6666`.

### Broadcast Mock Telemetry
...
## Client Game Configuration & Multi-Tenant Support
...
### Register the Streamer in MariaDB
...
### Configure the Streamer's PC (Remote / cPanel Deployment)
...
If you are running the bot locally for development, use the following config file at the same path:
