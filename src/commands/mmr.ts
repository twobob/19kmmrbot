...
export class MMRCommand implements TwitchCommand {
	...
	handler = async (
		...
		try {
			...
			let lobbyUser = matchState.players[user.steamid];
			if (!lobbyUser) {
				const nameLower = user.name.toLowerCase();
				lobbyUser = Object.values(matchState.players).find(p => 
					p.public_player_state.persona_name?.toLowerCase().includes(nameLower)
				);
			}
			...
			return client.say(
				channel,
				`Player: ${lobbyUser.public_player_state.persona_name} [#${player.rank}, MMR: ${player.mmr}]`,
			);
		} catch (e) {
